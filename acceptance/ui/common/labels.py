"""Общие подписи и компактные представления интерфейса пульта.
...
* `short_id` и `head_text` сокращают длинные значения для таблиц: идентификатор задачи
  нужен оператору редко, а описание занимает всю ширину. Полные значения всегда
  доступны в панели строки (см. экран «Задачи»), поэтому в таблице показывается
  «хвост» идентификатора и первые символы текста.
"""
