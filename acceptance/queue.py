...
(`SCR-102`) — новой ревизией программы. Инженер-испытатель может снять пункт с
...
* указатель прогона и «последний» пункт — для маркеров «следующая»,
  «выполняется», «последняя» на экране «Прогон» (`SCR-301`);
...
#: Маркеры пункта на экране «Прогон» (макет `docs/16`, `SCR-301`).
...
class Queue:
    ...
    def rows(self, results: Mapping[str, Any] | None = None) -> list[dict[str, Any]]:
        """Строки очереди для экрана «Прогон» (`SCR-301`).
